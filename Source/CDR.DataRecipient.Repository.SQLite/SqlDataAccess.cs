...
namespace CDR.DataRecipient.Repository.SQL
{
    public class SqlDataAccess : ISqlDataAccess
    {
        ...
        #endregion 
        public bool SqlCreateDatabase()
        {            
            using (SqlConnection db = new SqlConnection(_dbConn))
            {
                string dbProvider = _config.GetConnectionString("DBProvider");
                string dbName = _config.GetConnectionString("DBName");
                bool IsExits = CheckDatabaseExists(db, dbName);
                if (IsExits)
                {
                    // Create Tables
                    CreateTables(db);
                ...
                {
                    // Create Database THEN Create Tables
                    try
                    {
                        string tmpDbConn = string.Format("Server={0};Trusted_Connection=yes", dbProvider);
                        using (SqlConnection tmpConn = new SqlConnection(tmpDbConn))
                        {
                            tmpConn.Open();
                            tmpConn.ChangeDatabase("master");
                            string createDBQuery = string.Format("SELECT * FROM master.dbo.sysdatabases WHERE name = '{0}'", dbName);
                            bool created = false;
                            int exists = 0;
                            using (SqlCommand sqlCmd = new SqlCommand(createDBQuery, tmpConn))
                            {
                                exists = sqlCmd.ExecuteNonQuery();
                                if (exists <= 0)
                                {
                                    string createDatabase = string.Format("CREATE DATABASE [{0}]", dbName);
                                    using (SqlCommand dbCommand = new SqlCommand(createDatabase, tmpConn))
                                    {
        ...
        protected bool CheckDatabaseExists(SqlConnection testConn, string dbName)
        {
            ...
            try
            {
                ...
                using (SqlCommand sqlCmd = new SqlCommand(sqlCreateDBQuery, testConn))
                {
                    testConn.Open();
        ...
        protected bool CreateTables(SqlConnection db)
        {
            try
            {
                db.Open();
                var createTables = @"IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='CdrArrangement' AND xtype='U') CREATE TABLE CdrArrangement ([CdrArrangementId] [uniqueidentifier] NOT NULL, JsonDocument VARCHAR(MAX));
                ...
                SqlCommand tblCommand = new SqlCommand(createTables, db);
